...
namespace SimpleL7Proxy.ProxyWorker
{
    public class ProxyWorker
    {
        ...
        private BlockingPriorityQueue<RequestData.RequestData>? _requestsQueue;
        private readonly IBackendService _backends;
        private readonly BackendOptions.BackendOptions _options;
        ...
        public ProxyWorker(CancellationToken cancellationToken, BlockingPriorityQueue<RequestData.RequestData> requestsQueue, BackendOptions.BackendOptions backendOptions, IBackendService? backends, IEventHubClient? eventHubClient, TelemetryClient? telemetryClient)
        {
        ...
        public async Task TaskRunner()
        {
            ...
            {
                RequestData.RequestData incomingRequest;
                try
                {
        ...
        private async Task WriteResponseAsync(HttpListenerContext context, ProxyData.ProxyData pr)
        {
        ...
        public async Task<ProxyData.ProxyData> ReadProxyAsync(RequestData.RequestData request) //DateTime requestDate, string method, string path, WebHeaderCollection headers, Stream body)//HttpListenerResponse downStreamResponse)
        {
                        ...
                        using (var proxyResponse = await (_options?.Client ?? throw new ArgumentNullException(nameof(_options))).SendAsync(proxyRequest))
                        {
                            ...
                            if (((int)proxyResponse.StatusCode > 300 && (int)proxyResponse.StatusCode < 400) || (int)proxyResponse.StatusCode > 500)
                            {
                                ...
                                if (request.Debug)
                                {
                                    try
                                    {
                                        var temp_pr = new ProxyData.ProxyData()
                                        {
                            ...
                            // Read the response
                            var pr = new ProxyData.ProxyData()
                            {
                                ResponseDate = responseDate,
                                StatusCode = proxyResponse.StatusCode,
                                FullURL = request.FullURL,
            ...
            return new ProxyData.ProxyData
            {
                // 502 Bad Gateway
        ...
        private async Task GetProxyResponseAsync(HttpResponseMessage proxyResponse, RequestData.RequestData request, ProxyData.ProxyData pr)
        {
        ...
        private Encoding GetEncodingFromContentType(MediaTypeHeaderValue? contentType, RequestData.RequestData request)
        {
        ...
        private HttpStatusCode HandleError(BackendHost.BackendHost? host, Exception? e, DateTime requestDate, string url, HttpStatusCode statusCode, string? customMessage = null)
        {
